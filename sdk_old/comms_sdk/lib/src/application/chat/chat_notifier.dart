...
class ChatNotifier extends Notifier<ChatState> {
  ...
  Future<void> fetchChats(BuildContext context, String roleId) async {
    ...
    roleId = roleId;
    final userId = CoreLocalStorage.getUser()?.id;
    QuerySnapshot? query;
    try {
    ...
    if (query?.size == 0) {
      ...
      final res = await chats.add({
        ...
        "roleId": roleId,
        'user': {
          'firstname': CoreLocalStorage.getUser()?.firstname,
          'id': CoreLocalStorage.getUser()?.id,
          'img': CoreLocalStorage.getUser()?.img,
          'lastname': CoreLocalStorage.getUser()?.lastname,
        },
      });
  ...
  void checkAuthorised(BuildContext context) {
    // if (CoreLocalStorage.instance.getUserId() == null) {
    //   AppHelpers.showCheckTopSnackBar(
    //     context,
