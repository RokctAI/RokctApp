...
class LocalStorage {
  ...
  static Future<void> setLanguage(LanguageData? langData) async =>
      CoreLocalStorage.setLanguage(langData);

  static LanguageData? getLanguage() => CoreLocalStorage.getLanguage();

  static Future<void> setActiveLanguages(List<Language> languages) async {
    if (CoreLocalStorage.preferences != null) {
  ...
  static LanguageData? getSystemLanguage() {
    final lang = CoreLocalStorage.preferences?.getString(
      StorageKeys.keySystemLanguage,
    );
    if (lang == null) {
      return null;
