...
class OrderNotifier extends Notifier<OrderState> {
  ...
  Future<void> getCalculate({
    ...
    if (connected) {
      ...
      final response = await DependencyManager.ordersRepository.getCalculate(
        cartId: cartId,
        lat: lat,
        long: long,
        type: type,
  ...
  void setNotes({required String stockId, required String note}) {
    ...
    List<ProductNote> list = List<ProductNote>.from(state.notes);
    bool isAdd = true;
    int existingIndex = -1;
  ...
  Future<void> sendTips({
    ...
    if (payment.tag?.toLowerCase() != "cash") {
      final res = await DependencyManager.ordersRepository.tipProcess(
  ...
  Future createOrder({
    ...
    if (connected) {
      ...
      if (data.deliveryType == DeliveryTypeEnum.delivery) {
        final res = await DependencyManager.shopsRepository.checkDriverZone(
        ...
        res.when(
          success: (status) async {
            ...
            if (payment.tag != "cash" && payment.tag != "wallet") {
              final res = await DependencyManager.ordersRepository.process(
                data,
                payment.tag ?? '',
              );
              res.when(
      ...
      if (connected) {
        ...
        listOfProduct?.forEach((element) {
          for (Addons addon in element.addons ?? []) {
            list.add(
              CartRequest(
                stockId: addon.stocks?.id?.toString(),
                quantity: addon.quantity,
                parentId: element.stock?.id?.toString() ?? "",
              ),
            );
          }
          list.add(
            CartRequest(
              stockId: element.stock?.id?.toString() ?? "",
              quantity: element.quantity ?? 0,
            ),
          );
        });
        final response = await DependencyManager.cartRepository.insertCart(
          cart: CartRequest(
  ...
  Future<void> refundOrder(BuildContext context, String title) async {
    ...
    if (connected) {
      state = state.copyWith(isButtonLoading: true);
      final response = await DependencyManager.ordersRepository.refundOrder(
        '${state.orderData?.id ?? ""}',
        title,
      );
      response.when(
  ...
  Future<void> addReview(
    ...
    if (connected) {
      state = state.copyWith(isButtonLoading: true);
      final response = await DependencyManager.ordersRepository.addReview(
        state.orderData?.id ?? "",
        rating: rating,
        comment: comment,
      );
