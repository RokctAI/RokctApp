# API Reference: create_food_stocks_notifier

Source file: `lib/manager/application/foods/create/stocks/create_food_stocks_notifier.dart`
...
### `CreateFoodStocksState()); void setStockAddons(List<ProductData> addons, int stockIndex)`
*No documentation provided (generation failed).*

### `toggleCheckedGroup(int groupIndex)`
*No documentation provided (generation failed).*

### `fetchGroups() async { if (state.groups.isNotEmpty)`
*No documentation provided (generation failed).*
...
### `updateStocks(BuildContext context, { String? uuid, VoidCallback? updated, VoidCallback? failed, }) async { state = state.copyWith(isSaving: true); final response = await _productsRepository.updateStocks( deletedStocks: [], stocks: _localStocks, uuid: uuid, ); response.when( success: (data)`
*No documentation provided (generation failed).*
